...
6. GMGN 自建交易系统路线必须按阶段门推进：只读数据 → 样本反证 → 状态机 mapper → 回测/消融 → paper/quote-only → manual approval → limited auto。
```
...
## 7. P5 GMGN 自建量化交易系统架构

目标：

```text
把最终 GMGN 自建 meme 量化交易系统的系统层次、资料归位、缺口阶段门、MVP 闭环和迁移路线落入正式知识库。
```
...
```text
1. 13_量化交易系统架构 已进入 README / SOURCE_OF_TRUTH / 索引 / 变更记录。
2. 明确当前 MVP 是 observe_only / research_only，不是交易执行。
3. 后续实现只能从只读数据、schema、fixture、mapper 和 validator 开始。
```
...
4. 是否接受 13_量化交易系统架构 作为最终 GMGN 自建 meme 量化交易系统的系统级入口。
5. 是否继续把自动交易方向锁在阶段门之后，当前仅推进 observe_only / research_only MVP。
```
