...
subprotocol_activation_plan：not_applicable_reason：本历史报告产生时尚未引入 `intent-brainstorm-grill` 子协议激活态检查。
subprotocol_execution_evidence：not_applicable_reason：不回填伪执行；历史运行没有产出该子协议的 funnel artifact。
```
