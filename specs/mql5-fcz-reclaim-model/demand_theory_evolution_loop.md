...
如果 Step 2 发现需要外部搜索，不能直接拿 Owner 原话、中文口语或项目内部叫法去搜。必须先把问题转换为可检索的专业表达。

本步骤解决：

```text
vocabulary mismatch / vocabulary gap / term mismatch
...
```text
owner_surface_terms：Owner 原始表层词。
...
query_drift_risk：哪些扩展可能偏离 Owner 原意。
...
```text
1. 先解释 Owner 想找的“信息需求”，再生成搜索词。
2. 如果中文搜不到，应生成英文、平台语言或领域通用语。
3. 如果直接概念太窄，生成 step-back query 搜上位概念。
4. 如果问题包含多个检索目标，先 decomposition 成多个子查询。
5. 如果原话太口语，先 rewrite 成正式问题。
6. 如果领域文档通常用技术写法，生成 HyDE-style hypothetical document / answer paragraph 的关键词。
7. 每组查询必须标注它试图修复的 retrieval failure mode。
8. 不得把 Owner 原话机械复制成唯一搜索词。
```

本项目默认查询族示例：

```text
Owner 说“外部来源做补全” → knowledge integration / external knowledge absorption / evidence mapping。
Owner 说“我说的词可能不对” → vocabulary mismatch / query reformulation / query expansion。
Owner 说“中文搜不到可能语言不对” → cross-lingual information retrieval / multilingual query expansion。
Owner 说“搜到语义但没理解整体意思” → step-back prompting / problem decomposition / sensemaking。
Owner 说“论坛有没有成品拼接补全” → pattern mining / reference architecture / query transformation patterns。
```
...
MQL5 中文 Articles：https://www.mql5.com/zh/articles
...
```text
1. 形成中文/英文关键词后，优先执行 `site:mql5.com/zh/articles <关键词>` 检索。
2. 如果主题涉及 MQL5 指标、EA、回测、优化、数据导出、观察器 MVP 或平台机制，必须把 `https://www.mql5.com/zh/articles` 列入 external_sources_absorbed 或 not_applicable_reason。
3. 不能只搜泛 Web、Reddit 或 MQL5 forum；中文 Articles 是本项目 MQL5 实现证据的 P1 来源。
4. MQL5 Articles 可用于吸收实现方法、指标计算、回测纪律和平台机制；不得直接当作 FCZ 策略有效性证明。
```
